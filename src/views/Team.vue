      ...
      <div class="card-body" v-if="teammates && teammates.length">
        <template v-for="(item, index) in teammates">
          <div class="card team-view__member" :key="item._id">
            <div class="card-body">
              <router-link
                class="team-view__member-header"
                :to="{ name: 'member', params: { id: item._id } }"
              >
        ...
        <router-link class="btn btn-primary" to="/team/add"
...
export default {
  ...
  computed: {
    ...mapGetters(["projects", "teammates", "haveProjects", "workspace"]),
